  ...
  <div class="top-up-balance">
    ...
    <div class="cabinet__content">
      ...
      <div class="payment__methods-wrap mb-10">
        ...
        <div class="payment__methods-item mr-20" style="cursor: pointer"
             :class="[isMobileSmall ? 'pa-15' : 'pa-20', selectedPaymentType === 1 ? 'active' : '']"
        ...
        <div class="payment__methods-item mr-20" style="cursor: pointer"
             :class="[isMobileSmall ? 'pa-15' : 'pa-20', selectedPaymentType === 2 ? 'active' : '']"
...
export default {
  ...
  methods: {
    ...
    createPayment() {
      try {
        this.$api.post('main/User/AddPayment?Amount=' + this.paymentAmount).then(res => {
          if (!res.error) {
    ...
    makePaymentClick() {
      window.open('https://my.click.uz/services/pay?service_id='
          + this.clickRequest.service_id
          + '&merchant_id=' + this.clickRequest.merchant_id
          + '&amount=' + this.clickRequest.amount
          + '&transaction_param=' + this.clickRequest.transaction_param
          + '&return_url=' + this.clickRequest.return_url
          + '&card_type=' + this.clickRequest.card_type, '_self');
    },
    makePaymentPayme() {
